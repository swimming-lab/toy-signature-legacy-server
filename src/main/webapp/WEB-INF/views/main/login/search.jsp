...
<my:html cookie="YES" handlebars="YES" ajaxPage="YES">
            ...
            $('table tr').on('click', (event) => {
                ...
                <c:choose>
                    <c:when test="${mode != null && mode == 'direct'}">
                        MyCommon.goLink('/login/direct.view?customerId=' + customerId);
                    </c:when>
                    <c:otherwise>
                        MyCommon.goLink('/login/login.view?customerId=' + customerId);
                    </c:otherwise>
                </c:choose>
            });
    ...
    <script id="empty" type="text/x-handlebars-template">
       <div class="text-center">검색 결과가 없습니다. 신규 가입을 해주세요.</div>
       <div class="col-lg text-center" style="margin-bottom: 20px;" >
           <a class="btn btn-success btn-facebook" style="color:#fff" href="/login/join.view">
               신규가입
           </a>
       </div>
    </script>
