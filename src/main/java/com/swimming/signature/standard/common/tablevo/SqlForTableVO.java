...
public class SqlForTableVO {
    ...
    private static final String TABLE_COLUMN_NAME_UPD_ID = "UPD_ID";
    private static final String TABLE_COLUMN_NAME_UPD_DT = "UPD_DT";
    private static final String TABLE_COLUMN_NAME_CONN_DT = "CONN_DT";
    private static final String VARIABLE_NAME_REG_ID = "regId";
    private static final String VARIABLE_NAME_REG_DT = "regDt";
    private static final String VARIABLE_NAME_UPD_ID = "updId";
    private static final String VARIABLE_NAME_UPD_DT = "updDt";
    private static final String VARIABLE_NAME_CONN_DT = "connDt";
    ...
    public <T> String count(@NonNull final T vo, final Set<String> whereKeys) {
        ...
        sql.SELECT("COUNT(1) AS CNT").FROM(getTableName(vo.getClass().getSimpleName()).toLowerCase());
    ...
    public <T> String selectOne(@NonNull final T vo, @NonNull final Set<String> whereKeys) {
        ...
        sql.FROM(getTableName(vo.getClass().getSimpleName()).toLowerCase());
        this.getWhereSql(vo, whereKeys, sql);
        log.debug(sql.toString());
        return sql.toString();
    }
    ...
    public <T> String insert(@NonNull final T vo) {
        final Map<String, Object> param = MapperUtils.toHashMap(vo);
        final SQL sql = new SQL();
        sql.INSERT_INTO(getTableName(vo.getClass().getSimpleName()).toLowerCase());
        for (final Entry<String, Object> entry : param.entrySet()) {
            final String javaFieldName = entry.getKey();
            final String dbColumnName = this.getDbColumnName(javaFieldName);
            ...
            if (!StringUtils.equalsAny(javaFieldName, VARIABLE_NAME_REG_DT, VARIABLE_NAME_UPD_DT)) {
                final String columnTypeName = entry.getValue().getClass().getSimpleName();
                if (ENCRYPTED_COLUMN_LIST.contains(javaFieldName)) {
        ...
        for (final Field field : class1.getDeclaredFields()) {
            ...
            if (!EXCLUDE_FIELD_SET.contains(CaseFormat.LOWER_CAMEL.to(CaseFormat.UPPER_UNDERSCORE, javaFieldName))) {
                if (StringUtils.equals(javaFieldName, VARIABLE_NAME_REG_DT)) {
                    sql.VALUES(TABLE_COLUMN_NAME_REG_DT, SYSDATE);
                } else if (StringUtils.equals(javaFieldName, VARIABLE_NAME_UPD_DT)) {
                    sql.VALUES(TABLE_COLUMN_NAME_UPD_DT, SYSDATE);
                } else if (StringUtils.equals(javaFieldName, VARIABLE_NAME_CONN_DT)) {
                    sql.VALUES(TABLE_COLUMN_NAME_CONN_DT, SYSDATE);
                }
                else if (StringUtils.equals(javaFieldName, VARIABLE_NAME_REG_ID)) {
                    if (param.get(VARIABLE_NAME_REG_ID) == null) {
                        sql.VALUES(TABLE_COLUMN_NAME_REG_ID, "'" + SessionUtils.getCustomerId() + "'");
                    } else {
                        sql.VALUES(TABLE_COLUMN_NAME_REG_ID, "'" + param.get(VARIABLE_NAME_REG_ID) + "'");
                    }
                } else if (StringUtils.equals(javaFieldName, VARIABLE_NAME_UPD_ID)) {
                    if (param.get(VARIABLE_NAME_UPD_ID) == null) {
                        sql.VALUES(TABLE_COLUMN_NAME_UPD_ID, "'" + SessionUtils.getCustomerId() + "'");
                    } else {
                        sql.VALUES(TABLE_COLUMN_NAME_UPD_ID, "'" + param.get(VARIABLE_NAME_UPD_ID) + "'");
                    }
                }
            }
        }
    ...
    public <T> String update(@NonNull final T vo, @NotNull final Set<String> whereKeys, @Nullable final Set<String> forcedUpdateKeys) {
        ...
        final SQL sql = new SQL();
        sql.UPDATE(getTableName(vo.getClass().getSimpleName()).toLowerCase());
        for (final Field field : vo.getClass().getDeclaredFields()) {
            ...
            if (EXCLUDE_FIELD_SET.contains(dbColumnName)) {
                continue;
            } else if (StringUtils.equalsAny(javaFieldName, VARIABLE_NAME_REG_ID, VARIABLE_NAME_REG_DT, VARIABLE_NAME_UPD_ID, VARIABLE_NAME_UPD_DT)) {
                continue;
            }
            ...
            if (forcedUpdateKeys != null && !whereKeys.contains(javaFieldName) && (forcedUpdateKeys.contains("**") || forcedUpdateKeys.contains(javaFieldName))
                    && !StringUtils.equalsAny(javaFieldName, TABLE_COLUMN_NAME_REG_ID, TABLE_COLUMN_NAME_REG_DT)) {
                if (ENCRYPTED_COLUMN_LIST.contains(javaFieldName)) {
        ...
        for (final Field field : class1.getDeclaredFields()) {
            ...
            if (!EXCLUDE_FIELD_SET.contains(this.getDbColumnName(javaFieldName))) {
                if (StringUtils.equals(javaFieldName, VARIABLE_NAME_UPD_DT)) {
                    sql.SET(TABLE_COLUMN_NAME_UPD_DT + " = " + SYSDATE);
                } else if (StringUtils.equals(javaFieldName, VARIABLE_NAME_UPD_ID)) {
                    sql.SET(MessageFormat.format(SET_UPDATED_BY_BIND_STRING, TABLE_COLUMN_NAME_UPD_ID, SessionUtils.getCustomerId()));
                }
            }
        }
    ...
    public <T> String delete(@NonNull final T vo, final Set<String> whereKeys) {
        ...
        final SQL sql = new SQL();
        sql.DELETE_FROM(getTableName(vo.getClass().getSimpleName()).toLowerCase());
        this.getWhereSql(vo, whereKeys, sql);
        if (!StringUtils.containsIgnoreCase(sql.toString(), "WHERE ")) {
            log.warn("Not Found 'WHERE'");
            throw BusinessException.FAIL_SYSTEM;
